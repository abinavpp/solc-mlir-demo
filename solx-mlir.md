...
Below is a high-level overview of the solx-mlir compilation pipeline:
...
Comparing compile times:
  ...
  solx-mlir: solc --mlir-target=evm --mlir-action=print-asm --mlir-opt=3 <file>
  iterations: 20

File                 |    us-solc (s) |  solx-mlir (s) |     Difference |   Percent
-------------------------------------------------------------------------------------
...
solx-mlir generated bytecode uses relatively less gas. The following table is
...
The following summarizes the current state of solx-mlir:
